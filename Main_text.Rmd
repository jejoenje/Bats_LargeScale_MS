...
To account for expected confounding effects of habitat variation on bat activity along the transects, we used two key measures of habitat variability in each transect section: (1) edge density and (2) proportion of woodland. These two variables were selected on the basis of a preliminary analysis of the effect of a full set of `r n_hab_var` habitat variables on bat activity. This was done to avoid both overparameterisation of the main statistical models presented here, as well as the inclusion of highly collinear habitat metrics. Full details of this preliminary analysis are given in Appendix 1.
