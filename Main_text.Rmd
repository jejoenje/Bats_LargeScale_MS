...
In summary, based on the data presented here it is unlikely that SWTs in the range of 6-25m hub height have adverse effects on bat activity on a landscape (>100 m) scale, or that such effects are cumulative (when considering 2-4 installed turbines). Given the rapid expansion of the SWT sector both in the UK and worldwide, and the fact that in many cases planning- or siting permission is required, these findings are of key importance to both decision makers on the local level as well as to renewable energy policy makers.
